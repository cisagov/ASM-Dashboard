...
class Command(BaseCommand):
    help = (
        "Tests the get_user_domains function for a given user_id and prints the output."
    )

    def add_arguments(self, parser):
        parser.add_argument(
            "user_id", type=str, help="The ID of the user to retrieve domains for."
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if domains:
                ...
                self.stdout.write(
                    self.style.WARNING(f"No domains found for user_id {user_id}.")
                )
