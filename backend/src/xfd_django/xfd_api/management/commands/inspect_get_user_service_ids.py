...
from xfd_api.models import (  # Adjust the import paths as necessary
    Domain,
    Role,
    Service,
    User,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if service_ids:
            ...
            self.stdout.write(
                self.style.WARNING(f"No services found for user {user_id}.")
            )
    ...
    def get_user_service_ids(self, user_id):
        ...
        self.stdout.write(
            f"Organization IDs for user {user_id}: {list(organization_ids)}"
        )
