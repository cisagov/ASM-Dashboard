    ...
    for org in all_orgs:
        ...
        while not done:
            ...
            if response and response.get("status") == "Completed":
                shodan_asset_array = response.get("result", {}).get("data", {}).get("shodan_assets", [])
                shodan_vuln_array = response.get("result", {}).get("data", {}).get("shodan_vulns", [])
                ...
                save_findings_to_db(shodan_asset_array, shodan_vuln_array, org, shodan_datasource)
...
def fetch_dmz_shodan_task(org_acronym, page, per_page, since_timestamp):
    ...
    print("Fetching shodan vulnerability and asset task for organization: {acronym}".format(acronym=org_acronym))
    headers = {
...
def fetch_dmz_shodan_data(task_id):
    ...
    url = "https://api.staging-cd.crossfeed.cyber.dhs.gov/pe/apiv1/get_mdl_shodan_data/task/{t_id}".format(t_id=task_id)
    headers = {
        "X-API-KEY": os.getenv("CF_API_KEY"),
        "access_token": os.getenv("PE_API_KEY"),
        "Content-Type": "",
