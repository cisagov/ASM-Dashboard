        ...
        for key, value in update_dict.items():
            if key == 'origin_cidr':
                if value.id == ip_object.origin_cidr.id:
                    continue
                elif ip_object.origin_cidr.retired == True:
                    ...
                    if result == 'og':
                        ...
                        current_alerts.add('IP also associated with larger cidr {cidr}, cidr_id:{id}'.format(
                        ...
                        current_alerts = set(ip_object.conflict_alerts)
                        current_alerts.add('IP also associated with same sized cidr {cidr}, cidr_id:{id}'.format(
                            cidr=value.network, 
                            id=value.id
                        ))
                        ip_object.conflict_alerts = list(current_alerts)
